 ...
 *
 * This is a wrapper function to allow a safe conversion between Acconeer's start routine's signature
 * and the one required by the PThread Library
 *
 */
...
static acc_os_thread_handle_t acc_driver_os_thread_create(void (*func)(void *param), void *param, const char *name)
{
	...
	if (thread != NULL)
	{
		thread->info = (thread_t){func, param};

		ret = pthread_create(&thread->handle, NULL, thread_start, &thread->info);
		if (ret != 0)
		{
...
static bool acc_driver_os_semaphore_wait(acc_os_semaphore_t sem, uint16_t timeout_ms)
{
	...
	const time_t seconds = timeout_ms / 1000;
	ts.tv_sec  += seconds;
	timeout_ms -= seconds * 1000;

	ts.tv_nsec += (time_t)timeout_ms * 1000000;
	...
	while ((s = sem_timedwait(&(sem->handle), &ts)) == -1 && errno == EINTR)
	{
