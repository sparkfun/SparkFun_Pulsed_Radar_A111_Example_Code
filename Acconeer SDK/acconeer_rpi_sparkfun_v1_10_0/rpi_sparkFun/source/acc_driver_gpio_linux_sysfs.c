...
static bool internal_gpio_open(uint_fast8_t pin)
{
	...
	if (export_fd < 0)
	{
		ACC_LOG_ERROR("Unable to open gpio export: %s", strerror(errno));
		return false;
	}
...
static bool clear_interrupt(const gpio_t *gpio)
{
	...
	{
		ACC_LOG_ERROR("Failed to lseek GPIO.");
		return false;
	}
	...
	{
		ACC_LOG_ERROR("Failed to read GPIO.");
		return false;
	}
...
static void wait_for_interrupts(void *param)
{
	...
	while (is_isr_registered(gpio))
	{
		...
		if (ret > 0)
		{
			...
			{
				ACC_LOG_ERROR("Failed to clear interrupt!");
				return;
			}
		...
		{
			ACC_LOG_ERROR("An error occurred while waiting for interrupt on pin %" PRIuFAST8 ". Error: %s", gpio->pin, strerror(errno));
			return;
		}
	}
}
