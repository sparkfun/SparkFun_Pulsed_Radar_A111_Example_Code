 ...
 *
 * In the IQ service, the base step length is ~0.5mm. Thus, for example setting stepsize to 4 makes
 ...
 */
uint16_t acc_service_iq_stepsize_get(acc_service_configuration_t service_configuration);
 ...
 * the distance between two points in the measured range ~2mm.
 *
 * The IQ service supports a stepsize of 1, 2, or 4.
 *
 * @param[in] service_configuration The configuration to set stepsize in
 * @param[in] stepsize The stepsize
 */
void acc_service_iq_stepsize_set(acc_service_configuration_t service_configuration, uint16_t stepsize);
