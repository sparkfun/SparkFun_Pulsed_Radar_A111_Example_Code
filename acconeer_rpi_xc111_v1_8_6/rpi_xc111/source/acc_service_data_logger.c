...
static acc_service_status_t execute_power_bin(acc_service_configuration_t power_bin_configuration, char *file_path, bool wait_for_interrupt,
                                              uint16_t sweep_count);
...
static acc_service_status_t execute_envelope(acc_service_configuration_t envelope_configuration, char *file_path, bool wait_for_interrupt,
                                             uint16_t sweep_count);
...
static void print_usage(void)
{
	...
	printf("-f, --frequency           sweep frequency, default %" PRIfloat "\n",
...
acc_service_status_t execute_power_bin(acc_service_configuration_t power_bin_configuration, char *file_path, bool wait_for_interrupt,
                                       uint16_t sweep_count)
{
	...
	if (service_status == ACC_SERVICE_STATUS_OK)
	{
		...
		while ((wait_for_interrupt && interrupted == 0) || sweeps < sweep_count)
		{
			...
			if (service_status == ACC_SERVICE_STATUS_OK)
			{
				...
				{
					fprintf(file, "%u\t", (unsigned int)(power_bins_data[index] + 0.5f));
				}
...
acc_service_status_t execute_envelope(acc_service_configuration_t envelope_configuration, char *file_path, bool wait_for_interrupt,
                                      uint16_t sweep_count)
{
